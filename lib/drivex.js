...
DriveX.prototype = {
   ...
   * @returns {Promise} resolves to true or throw error
   */
  waitForVisibleElement: function (locator, timeout, msg) {
    var that = this;
    log('waitForVisibleElement', locator);
    return this.driver.wait(this.wd.until.elementIsVisible(this.by(locator)), timeout, msg).then(function() {
      log('waitForVisibleElement: found', locator);
      return true;
    }, function(err) {
      log('waitForVisibleElement', err);
      log(err.stack);
      throw new Error(msg || '[drivex.waitForVisibleElement] Element not visible');
    });
  },
  /**
